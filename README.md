...
> ```
>
> The declarations are generated by the OpenAPI Generator from the `*.d.ts.mustache` templates in [`libTemplate/`](/libTemplate/) (wired in through the `files` block in [`config.json`](config.json)), then given a small mechanical clean-up by [`.github/scripts/generate-types.js`](.github/scripts/generate-types.js) — all run as part of `pnpm run generateDocs`. Because the types come from the same model the generator uses to emit the runtime code, they stay in sync with each release.
...
> ```
>
> **NOTE:** `libTemplate/` also contains the TypeScript declaration templates (`model.d.ts.mustache`, `api.d.ts.mustache`, `index.d.mustache`, `ApiClient.d.mustache`, and the shared `partial_ts_*.mustache` partials). The JavaScript generator does not emit types, so these templates produce a `.d.ts` for each generated `.js` from the same model metadata, keeping the declarations in lockstep with the runtime. They are wired in through the `files` block in `config.json`. (OpenAPI Generator collapses the `.d.ts` double extension for Model/API templates, so those emit `.ts` and `generate-types.js` renames them; `index.d.ts`/`ApiClient.d.ts` use SupportingFiles templates that keep their names.)

- The [`config.json`](config.json) file is a configuration file of additional properties used by the OpenAPI Generator and its JavaScript library template to define certain variables like license type, project name, project version, etc. Its `files` block registers the extra TypeScript declaration templates described above.
